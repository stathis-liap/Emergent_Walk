...
class CubicHermiteSpline:
    ...
    def interpolate(self, theta_q, theta_v, times):
        """
        Batched Interpolation (Fully Vectorized).
        ...
        # Vectorize time clamping and node selection
        times_arr = np.asarray(times)
        t_clamped = np.clip(times_arr, 0.0, T_max)
        
        k = np.clip((t_clamped // self.dt_node).astype(int), 0, K_nodes - 2)
        t_k = k * self.dt_node
        s = (t_clamped - t_k) / self.dt_node

        # Compute basis functions for all times at once (Shape: T)
        h00, h10, h01, h11 = self._basis_functions(s)
        dh00, dh10, dh01, dh11 = self._derivative_basis_functions(s)

        # Reshape for broadcasting to (1, T, 1) against (Batch, T, Joints)
        h00 = h00[None, :, None]
        h10 = h10[None, :, None]
        h01 = h01[None, :, None]
        h11 = h11[None, :, None]
        ...
        # Advanced indexing to fetch nodes for all times at once (Shape: Batch, T, Joints)
        q_k, v_k   = theta_q[:, k, :], theta_v[:, k, :]
        q_k1, v_k1 = theta_q[:, k+1, :], theta_v[:, k+1, :]

        # Fully vectorized interpolation (No Python loops)
        dense_q = (h00 * q_k) + (h10 * self.dt_node * v_k) + \
                  (h01 * q_k1) + (h11 * self.dt_node * v_k1)

        dense_v = (dh00 * q_k / self.dt_node) + (dh10 * v_k) + \
...
def apply_derivative_clamp_batch(theta_q_batch, theta_v_raw_batch, q_min, q_max, dt_node):
    """Vectorised version: inputs are (N, K, 12) arrays. No Python loops."""
    dist_to_max = q_max - theta_q_batch
    dist_to_min = theta_q_batch - q_min
    v_bound = np.minimum(dist_to_max, dist_to_min) / (dt_node / 2.0)
    return np.clip(theta_v_raw_batch, -v_bound, v_bound)
